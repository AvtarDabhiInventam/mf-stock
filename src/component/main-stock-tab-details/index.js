...
const MainStockTabDetails = ({ STOCK_DATA }) => {
  const stockGraph = {
    series: [
      {
        data: [
  ...
  return (
    <>
      <div className="ms-4 my-5">
        ...
        <div className={`${style.stock_detail_price} fw-bold`}>
          {STOCK_DATA.price}
        </div>
        <span
          className={`${style.stock_detail_price_updown} ${STOCK_DATA?.colorstatus}`}
        >
          {STOCK_DATA.result}
        </span>
        <span
...
export default MainStockTabDetails;
